...
require_once("cutdemo/image.class.1.5.php");
$image = $_POST['image'];
...
if(count($article_pic)>=6){	
	echo "<span style='color:red'>添加失败！您最多只能添加3张图片</span><br/>";
}
else{
	$image = $_POST['image'];
...
<script type="text/javascript">
    ...
    jQuery(function($){
      ...
      $('#target').Jcrop({
		minSize: [120,80],//比例：1
		setSelect: [0,0,240,160],//比例：2
        onChange: updatePreview,
        onSelect: updatePreview,
		onSelect: updateCoords,
        aspectRatio: 1.5//比例：3
      },
	function(){
...
?>	
	...
	<div style="width:120px;height:80px;margin:10px;overflow:hidden; float:left;"><img  style="float:left;" id="preview" src="<?php echo $image;?>" ></div><!--比例：6-->
	<div style="width:300px;height:200px;margin:10px;overflow:hidden; float:left;"><img  style="float:left;" id="preview2" src="<?php echo $image;?>" ></div><!--比例：7-->
<div style="float:left">	
    	<input type="hidden" id="x" name="x" />
		<input type="hidden" id="y" name="y" />
		<input type="hidden" id="w" name="w" />
		<input type="hidden" id="h" name="h" />
		<input type="submit" value="裁剪并添加" name="cut" style=" margin:100px 0 0 5px"/>
</div>
<?php
	}
}
